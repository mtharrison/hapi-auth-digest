...
it('returns a reply on successful auth', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        server.auth.strategy('default', 'digest', 'required', { validateFunc: internals.user });
        server.route({
            method: 'POST',
            path: '/',
...
it('returns an error on bad header format', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        var request = { method: 'POST', url: '/', headers: { authorization: 'digest' } };
...
it('returns an error on bad header internal syntax', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        var request = { method: 'POST', url: '/', headers: { authorization: 'digest 123' } };
...
it('allow missing username', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        server.auth.strategy('default', 'digest', {
...
it('should ask for credentials if server has one default strategy', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        server.auth.strategy('default', 'digest', { validateFunc: internals.user });
        server.route({
            path: '/',
            method: 'GET',
            config: {
...
it('passes non-error err in response', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        server.auth.strategy('digest', 'digest', true, {
            validateFunc: function (request, username, password, callback) {
...
it('accepts request object in validateFunc', function (done) {
    ...
    server.register(require('../'), function (err) {
        ...
        expect(err).to.not.exist();
        server.auth.strategy('default', 'digest', 'required', {
...
internals.header = function (username, password) {
    ...
    return 'Digest ' + (new Buffer(username + ':' + password, 'utf8')).toString('base64');
};
